  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
      <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_16px_50px_rgba(15,23,42,0.04)]">
        <p className="text-sm uppercase tracking-[0.2em] text-slate-400">Demande</p>
        ...
        <div className="mt-4 rounded-[22px] border border-slate-200 bg-slate-50/60 p-5">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Origine</p>
          <p className="mt-2 font-semibold text-slate-950">
            {appointment.origin === "administrateur"
              ? `Rendez-vous créé par un administrateur${appointment.createdByAdminEmail ? ` : ${appointment.createdByAdminEmail}` : ""}`
              : "Rendez-vous pris directement par l'utilisateur sur le site"}
          </p>
        </div>

        <div className="mt-6 rounded-[22px] border border-slate-200 bg-slate-50/60 p-5">
